...
BackpackName < Label
  ...
  Button
    ...
    width: 15
    height: 15
    tooltip: Clean Items in this container configuration.
...
if rootWidget then
    ...
    contListWindow.clearConfig.onClick = function(widget)
        config.list = {} -- Limpa a lista na memória/storage
        contListWindow.itemList:destroyChildren() -- Limpa a lista visual
        contListWindow.contId:setItemId(0) -- Reseta o ID do item
        contListWindow.contName:setText('') -- Reseta o nome
        contListWindow.sortList:setItems({}) -- Limpa a lista de itens para sort
    end
    ...
    local function refreshSortList(k, t)
        ...
        -- Garante visualmente que, ao carregar, o primeiro slot seja o input (id 0)
        -- Se a lista estiver vazia ou o primeiro item não for o slot vazio, insere ele.
        if #t == 0 or (t[1] and t[1].id ~= 0) then
            table.insert(t, 1, {id = 0, count = 1})
        end

        UI.Container(function()
            -- Pega o estado atual do container (o que você acabou de arrastar)
            local widgetItems = contListWindow.sortList:getItems()
            local newConfigItems = {}

            -- 1. [INTERCEPTAÇÃO] Sempre inicia a nova lista com o Slot Vazio {id: 0, count: 1}
            table.insert(newConfigItems, {id = 0, count = 1})

            -- 2. Percorre os itens que estão no widget para salvar os reais
            for _, item in ipairs(widgetItems) do
                -- Filtra apenas itens reais (ID > 100) para evitar duplicar o slot vazio ou lixo
                if item.id > 100 then
                    -- 3. Insere o item na lista mantendo o ID, mas FORÇANDO o count para 1
                    table.insert(newConfigItems, {id = item.id, count = 1})
                end
            end

            -- Salva na configuração global do bot se 'k' for válido
            if k and config.list[k] then
                config.list[k].items = newConfigItems
            end
            
            -- [ATUALIZAÇÃO COM SCHEDULE]
            -- Agenda a atualização do widget para ocorrer logo após o evento de drag&drop terminar.
            schedule(10, function()
                contListWindow.sortList:setItems(newConfigItems)
            end)
            
            end, true, nil, contListWindow.sortList) 
        
        -- Define os itens iniciais ao abrir a janela
    ...
    local refreshContNames = function(tFocus)
        ...
        if storageVal and #storageVal > 0 then
            ...
            for k, entry in pairs(storageVal) do
                ...
                label.clean.onClick = function(widget)
                    -- Limpa a lista de itens deste container específico
                    entry.items = {} 
                    
                    -- Se o container limpo for o que está sendo editado no momento, atualiza a lista visual
                    if contListWindow.contId:getItemId() == entry.item then
                        refreshSortList(k, entry.items)
                    end
                end
...
local function properTable(t)
    ...
    for _, entry in pairs(t) do
      if type(entry) == "number" then
        ...
        -- IMPORTANTE: Filtra o item ID 0 (slot vazio) para não tentar mover
        if entry.id ~= 0 then
