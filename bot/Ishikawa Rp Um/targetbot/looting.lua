...
TargetBot.Looting.setup = function()
  ...
  ui.clearLoot.onClick = function()
    local emptyList = {{id=0, count=1}} -- Reseta apenas para o slot de input
    dontSave = true
    ui.items:setItems(emptyList)
    dontSave = false
    TargetBot.save()
...
TargetBot.Looting.onItemsUpdate = function()
  ...
  -- [INTERCEPTAÇÃO] Lógica do Input Slot
  local widgetItems = ui.items:getItems()
  local newConfigItems = {}

  -- 1. Sempre inicia a lista com o Slot Vazio {id: 0, count: 1}
  table.insert(newConfigItems, {id = 0, count = 1})

  -- 2. Filtra e adiciona os itens reais
  for _, item in ipairs(widgetItems) do
    if item.id > 100 then -- Ignora IDs inválidos ou o próprio slot vazio anterior
        table.insert(newConfigItems, {id = item.id, count = 1}) -- Força count 1
    end
  end

  -- 3. Atualiza a UI e Salva (com agendamento para evitar conflito visual)
  schedule(10, function()
      dontSave = true
      ui.items:setItems(newConfigItems)
      dontSave = false
...
TargetBot.Looting.update = function(data)
  ...
  -- Garante que o slot de input exista ao carregar o perfil
  local itemsData = data['items'] or {}
  if #itemsData == 0 or itemsData[1].id ~= 0 then
      table.insert(itemsData, 1, {id=0, count=1})
  end
  ui.items:setItems(itemsData)
...
TargetBot.Looting.updateItemsAndContainers = function()
  ...
  for i, item in ipairs(items) do
    if item.id > 100 then -- Apenas itens reais vão para a tabela de busca
        itemsById[item.id] = 1
    end
  end
...
TargetBot.Looting.process = function(targets, dangerLevel)
  -- Verifica se existem itens reais para lootear (ignora o slot de input id 0)
  local hasItems = false
  for _, item in ipairs(items) do
    if item.id > 100 then hasItems = true break end
  end

  if (not hasItems and not ui.everyItem:isOn()) or not containers[1] then
    status = ""
    return false
  end
