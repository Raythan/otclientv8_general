...
edit.cleanTrash.onClick = function(widget)
  config.trashItems = {}
  edit.TrashItems:setItems(config.trashItems)
end

edit.cleanUse.onClick = function(widget)
  config.useItems = {}
  edit.UseItems:setItems(config.useItems)
end

edit.cleanCap.onClick = function(widget)
  config.capItems = {}
...
macro(200, function()
    ...
    local tables = {properTable(config.trashItems), properTable(config.useItems), properTable(config.capItems)}
            ...
            for __, item in ipairs(container:getItems()) do
                for ___, userItem in ipairs(tables[i]) do
                    if item:getId() == userItem then
                        return i == 3 and freecap() < 150 and dropItem(item) or
                               i == 2 and use(item) or
							   i == 1 and dropItem(item)
                    end
                end
            end
        end
