...
-- if true then
  ...
  -- if voc() == 1 or voc() == 5 or voc() == 11 then
      -- vocText = "- EK"
  -- elseif voc() == 2 or voc() == 6 or voc() == 12 then
      -- vocText = "- RP"
  -- elseif voc() == 3 or voc() == 7 or voc() == 13 then
      -- vocText = "- MS"
  ...
  -- end)
-- end
-- Verifica se a opção de título personalizado está ativada
if true then
  -- Define o macro para atualizar o título da janela
  macro(5000, function()
    -- Verifica se a opção de título personalizado está ativada
    if settings.title then
		-- Calcula as horas e minutos a partir do valor de stamina em minutos
		local hours = math.floor(stamina() / 60)
		local minutes = stamina() % 60

		-- Formata a string da stamina com dois dígitos para as horas e minutos
		local staminaText = string.format("%02d:%02d", hours, minutes)
		-- local staminaText = os.date("%H:%M", stamina())
		local titleText = ""

      -- Verifica se o jogador está vivo e atualiza o título com o formato desejado
      if hppercent() > 0 then
          titleText = name() .. " - " .. lvl() .. "lv " .. " - Stamina: " .. staminaText
      else
          titleText = name() .. " - DEAD"
      end

      g_window.setTitle(titleText) -- Define o título da janela
    else
      g_window.setTitle(name()) -- Define um título padrão caso a opção esteja desativada
    end
  end)
end
...
if true then
  ...
  macro(500, function()
      ...
      for i, tile in ipairs(g_map.getTiles(posz())) do
        ...
        if item and item:isContainer() and getDistanceBetween(pos(), tile:getPosition()) <= 1 then
